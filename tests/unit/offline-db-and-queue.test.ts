...
describe('offline db and queue', () => {
  ...
  it('initializes db once and creates tasks store when missing', async () => {
    ...
    expect(db.createObjectStore).toHaveBeenCalledWith('tasks', { keyPath: 'id' })

    const { addtoQueue, getQueue, clearQueue } = await import('@/lib/offline-queue')
    await addtoQueue({ id: '1', title: 't', status: 'TODO', projectId: 'p', priority: 'LOW', createdAt: '', updatedAt: '' })
    await getQueue()
    await clearQueue()

    expect(db.put).toHaveBeenCalledWith('tasks', expect.objectContaining({ id: '1' }))
